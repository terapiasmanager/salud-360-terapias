...
document.getElementById('dynamicTestForm').addEventListener('submit', (e) => {
    e.preventDefault();
    const config = testsConfig[currentTestId];
    const p = patients.find(x => x.id === currentPatientId);
...
document.getElementById('sessionForm').addEventListener('submit', (e) => {
    ...
    // VALIDACIÓN DE FIRMA
    const firmaBase64 = captureSignatureToBase64(signatureCanvas);
    if (!firmaBase64) {
        alert('La firma es obligatoria. Por favor, registre la firma antes de continuar.');
        return;
    }
...
function renderVisitas() {
        ...
        <div class="session-card">
            <div class="session-header">
                ...
                <div class="session-meta">
                    ...
                    <button class="action-btn delete" onclick="deleteVisita(${s.num})" style="margin-left: 10px; font-size: 0.8rem;" title="Eliminar registro">🗑️</button>
                </div>
            </div>
    ...
    if (printTbody) {
        if (!p.visitas || p.visitas.length === 0) {
            printTbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 20px;">No hay visitas registradas para imprimir.</td></tr>';
        } else {
...
function prepareAndPrint() {
    ...
    if (printTbody) {
        if (visitas.length === 0) {
            printTbody.innerHTML = '<tr><td colspan="7" style="text-align: center; padding: 30px; font-weight: bold;">SIN VISITAS REGISTRADAS</td></tr>';
        } else {
...
document.getElementById('articuloForm').addEventListener('submit', (e) => {
    ...
    // Validar firma usando validación por píxeles (más robusta)
    const firmaBase64 = captureSignatureToBase64(artCanvas);
    if (!firmaBase64) {
        alert('⚠️ La firma de recepción es obligatoria. Por favor, dibuje la firma en el recuadro antes de continuar.');
        return;
    }
...
function prepareArticuloPrint(data, p) {
    ...
    document.getElementById('partFirmanteRel').textContent = data.relacion;
}
